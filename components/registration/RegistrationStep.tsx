...
import { STEP_META } from "@/types/registration";
...
/**
 * Phase 11: the "royal event registry" frame around whichever step is
 * currently active. Renders whatever step content `app/register/page.tsx`
 * passes as `children`; the actual step components (ParticipantStep,
 * EventsStep, ...) keep their own `"use client"` boundaries exactly as
 * Phase 02 left them.
 *
 ...
 * adding it to the Tab order, the same pattern already used for
 * `DetailsStep`'s per-event error-scroll target. The heading's
 * `aria-label` includes the "Step N of 06" context that's otherwise only
 * in a sibling `<span>`, so a screen reader announces the full context in
 * one utterance on focus — mirroring `RegistrationProgress`'s own
 * `aria-label` pattern for the same information.
 */
  ...
  return (
    <OrnamentalFrame padding="lg" className="mt-8">
      <header className="mb-8 flex flex-col gap-1 border-b border-antique-gold/20 pb-6">
        <span className="font-accent text-sm italic text-warm-gold">Step {meta.number} of 06</span>
        <h2
          ref={headingRef}
          tabIndex={-1}
          aria-label={`Step ${meta.number} of 06: ${meta.label}`}
